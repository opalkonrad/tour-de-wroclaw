...
def get_attraction_pairs(
    network: nx.Graph, min_len: float = 1.0, max_weld_distance: float = 0.1
) -> List[Tuple[AttractionNode, AttractionNode, int]]:
...
def check_if_weld(
    attractions_pairs: List[Tuple[AttractionNode, AttractionNode, int]],
    new_pair: Tuple[AttractionNode, AttractionNode],
    max_d: float,
) -> Optional[int]:
    a1, a2 = new_pair
    coords1 = a1.pos
    ...
    for i, pair in enumerate(attractions_pairs):
        ...
        coords3 = a3.pos
        coords4 = a4.pos
        if (haversine(*coords1, *coords3) < max_d and haversine(*coords2, *coords4) < max_d) or (
